...
#define RCIN_RPI_MAX_COUNTER     (RCIN_RPI_BUFFER_LENGTH * PAGE_SIZE / 2) // 1/4 circle_buffer
...
void RCInput_RPI::_timer_tick()
{
    ...
    if (!ad) {
        fprintf(stderr, "DMA RCInput sampling stopped, restarting...\n");
        init_ctrl_data();
        init_PCM();
        init_DMA();
        return;
    ...
    if (counter > RCIN_RPI_MAX_COUNTER) {
        fprintf(stderr, "RCInput %5d sample(s) dropped\n", (counter - RCIN_RPI_MAX_COUNTER) / 0x8);
